...
import { mediaService } from "@/api/services/media.js";
...
const updateMagnifier = async (x, y) => {
  ...
  try {
    ...
    const maxSourceX = Math.max(
      0,
      fullScreenImage.value.naturalWidth - sourceRectSize
    );
    const maxSourceY = Math.max(
      0,
      fullScreenImage.value.naturalHeight - sourceRectSize
    );
...
const captureAndUpload = async (x, y, width, height) => {
  try {
    ...
    const uploadResult = await mediaService.uploadScreenshot(file, {
      title: `Screenshot ${new Date().toLocaleString()}`,
      onUploadProgress: (progressEvent) => {
        const progress = Math.round(
          (progressEvent.loaded * 100) / progressEvent.total
        );
        const notif = uploadNotifications.value.find(
    ...
    if (notif) {
      if (uploadResult.success) {
        ...
        notif.url =
          uploadResult.media?.url || uploadResult.media?.public_url || "#";
      } else {
        notif.status = "failed";
        notif.error = uploadResult.error || "Upload failed";
      }
    }
...
const calculateRemainingTime = (progressEvent) => {
  ...
  const bytesPerSecond =
    progressEvent.loaded / ((Date.now() - progressEvent.timeStamp) / 1000);
...
<template>
  <div
    class="fixed top-0 left-0 w-screen h-screen cursor-crosshair select-none"
  ...
  >
    ...
    <div
      ...
      class="absolute top-0 left-0 w-full h-full bg-black/50 pointer-events-none"
      :style="{
    ...
    <div
      v-if="mode !== 'idle'"
      :class="[
        'absolute animated-dashed-border',
        mode === 'confirming' || mode === 'resizing'
          ? 'pointer-events-all'
          : 'pointer-events-none',
      ]"
      :style="{
    ...
    >
      ...
      <div v-if="mode === 'confirming' || mode === 'resizing'">
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all -top-[5px] -left-[5px] cursor-nwse-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'top-left')"
        ></div>
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all -top-[5px] left-1/2 -translate-x-1/2 cursor-ns-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'top')"
        ></div>
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all -top-[5px] -right-[5px] cursor-nesw-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'top-right')"
        ></div>
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all top-1/2 -right-[5px] -translate-y-1/2 cursor-ew-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'right')"
        ></div>
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all -bottom-[5px] -right-[5px] cursor-nwse-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'bottom-right')"
        ></div>
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all -bottom-[5px] left-1/2 -translate-x-1/2 cursor-ns-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'bottom')"
        ></div>
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all -bottom-[5px] -left-[5px] cursor-nesw-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'bottom-left')"
        ></div>
        <div
          class="absolute w-[10px] h-[10px] bg-white border border-black rounded-full pointer-events-all top-1/2 -left-[5px] -translate-y-1/2 cursor-ew-resize"
          @mousedown.stop="handleResizeHandleMouseDown($event, 'left')"
        ></div>
      </div>
    </div>
    ...
    <div
      v-if="mode === 'idle'"
      class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/80 text-white py-2.5 px-4 rounded-lg text-center text-sm pointer-events-none z-[100]"
    >
    ...
    <div
      v-if="mode !== 'confirming'"
      class="fixed left-0 right-0 h-px bg-gray-900/50 pointer-events-none z-[99] transition-none"
      :style="{ top: mouseY + 'px' }"
    />
    <div
      v-if="mode !== 'confirming'"
      class="fixed top-0 bottom-0 w-px bg-gray-900/50 pointer-events-none z-[99] transition-none"
    ...
    <div
      ...
      class="fixed w-[200px] h-[200px] rounded-full border-2 border-white bg-black overflow-hidden shadow-[0_5px_15px_rgba(0,0,0,0.3)] pointer-events-none z-[101] flex justify-center items-center"
    ...
    >
      ...
      <div
        ...
        class="absolute -top-[30px] left-1/2 -translate-x-1/2 bg-black/80 text-white px-2 py-1 rounded text-xs whitespace-nowrap"
      >
        <p>
          {{ selectionDimensions.width }} x {{ selectionDimensions.height }}
        </p>
      </div>
    </div>
    ...
    >
      <div class="flex items-center rounded-full bg-white/90">
        <button
          @click="handleUpload"
          title="Upload"
          class="group flex cursor-pointer border border-transparent hover:border-white gap-2.5 rounded-full px-3.5 hover:px-5 py-3 transition-all hover:bg-primary-blue"
        >
          <svg
    ...
    <div class="fixed top-4 right-4 z-[10000] space-y-3 min-w-[380px]">
      <transition-group name="notification" tag="div">
        <div
          v-for="notification in uploadNotifications"
          :key="notification.id"
          class="bg-white rounded-lg shadow-2xl p-4 border border-gray-100"
        >
          <!-- Uploading State -->
              ...
              <div class="flex items-center gap-3">
                ...
                >
                  <svg
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M9 17H15V22H9V17Z" fill="#3B82F6" />
                    <path d="M12 2L7 7H10V15H14V7H17L12 2Z" fill="#3B82F6" />
                  </svg>
                </div>
                <div>
                  <h4 class="text-gray-900 font-medium">Uploading</h4>
                </div>
              </div>
          ...
          <div v-else-if="notification.status === 'completed'">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4 class="text-gray-900 font-medium">Upload Complete</h4>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <button
            ...
            <div class="space-y-3">
              ...
              >
                <a
                  ...
                  target="_blank"
                  class="text-blue-600 hover:text-blue-700 text-sm truncate flex-1 mr-2"
                >
                  {{ notification.url }}
                </a>
              ...
              <button
                @click="removeNotification(notification.id)"
                class="w-full bg-gray-900 text-white py-2 px-4 rounded-lg hover:bg-gray-800 transition-colors text-sm font-medium"
              >
                Copy Link
              </button>
            </div>
          ...
          <div v-else-if="notification.status === 'failed'">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4 class="text-gray-900 font-medium">Upload Failed</h4>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <button
            ...
            <div class="space-y-3">
              <div class="bg-red-50 rounded-lg p-3 flex items-center gap-2">
                <div
