...
import { WEB_URLS } from "@/api/config";
...
const viewUploadedHistory = () => {
  store.openExternal(WEB_URLS.BASE_URL + WEB_URLS.CAPTURES);
  isUserMenuOpen.value = false;
  hideWindow("main");
};

const visitProfile = () => {
  store.openExternal(WEB_URLS.BASE_URL + WEB_URLS.PROFILE);
  isUserMenuOpen.value = false;
  hideWindow("main");
};

const showHelp = () => {
  store.openExternal(WEB_URLS.BASE_URL + WEB_URLS.HELP_CENTER);
  isUserMenuOpen.value = false;
  hideWindow("main");
};
...
<template>
  <div
    class="main-container flex flex-col h-full bg-white dark:bg-dark-blue select-none px-4 py-6 rounded-2xl w-fit"
  >
    <!-- Capture Actions -->
    <div class="space-y-4">
      <button
        ...
        class="w-full flex items-center gap-6 px-3 py-2 text-gray-700 dark:text-gray-200 rounded-lg hover:bg-gray-200/10 dark:hover:bg-dark-800 transition-colors group"
      >
        <svg
          width="28"
          height="25"
        ...
        <p
          class="font-bold text-gray-black text-sm dark:text-white dark:group-hover:text-primary-blue"
        >
          Take Screenshot
      ...
      <button
        type="button"
        @click="recordVideo"
        class="w-full flex items-center gap-6 px-2.5 py-1.5 text-gray-700 dark:text-gray-200 rounded-lg hover:bg-gray-200/10 dark:hover:bg-dark-800 transition-colors group"
      >
        <svg
          width="29"
          height="30"
    ...
    <hr class="border-gray-400 dark:border-dark-700 my-4" />
    ...
    <div class="space-y-4">
      ...
      >
        <svg
          ...
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M2.41699 9.13501H4.84574H7.78199" fill="#2178FF" />
          <path
        ...
        <p
          class="text-gray-black text-sm dark:text-white dark:group-hover:text-primary-blue"
        >
          Last Capture
        </p>
      </button>
    ...
    <div class="space-y-6">
      ...
      <!-- User Profile -->
      <div class="relative flex items-center gap-3 px-2.5 py-1.5 rounded-2xl">
        <img
          class="w-10 h-10 rounded-full cursor-pointer object-cover ring-2 ring-primary-blue shadow-lg"
          @click="visitProfile"
          :src="store.user?.profile_photo_url"
          alt="User avatar"
        />

        <div class="flex-1 cursor-pointer" @click="visitProfile">
        ...
        <button
          ...
          class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 px-4 py-2 hover:bg-gray-200/10 rounded-lg"
        >
          <svg
            width="5"
            height="23"
            ...
            xmlns="http://www.w3.org/2000/svg"
          >
            <circle cx="2.5" cy="2.5" r="2.5" fill="#2178FF" />
            <circle cx="2.5" cy="11.5" r="2.5" fill="#2178FF" />
            <circle cx="2.5" cy="20.5" r="2.5" fill="#2178FF" />
          </svg>
        </button>
        ...
        <div
          v-if="isUserMenuOpen"
          class="absolute right-0 bottom-12 w-48 bg-gray-100 dark:bg-dark-800 rounded-lg shadow-2xl py-2 z-10"
        >
          <a
            @click="showHelp"
            href="#"
            class="flex items-center gap-3 px-4 py-2 text-gray-700 text-sm hover:bg-gray-200/10 dark:hover:bg-dark-700 dark:text-white dark:group-hover:text-primary-blue"
          >
            <svg
              width="29"
              height="29"
          ...
          <hr class="border-gray-400 dark:border-dark-700 my-2" />
          <a
            @click="handleLogout"
            href="#"
            class="flex items-center gap-3 px-4 py-2 text-sm text-red-500 hover:bg-gray-200/10 dark:hover:bg-dark-700"
          >
            <svg
              width="29"
              height="29"
