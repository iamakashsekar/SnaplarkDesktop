...
// Ensure single instance on Windows and Linux
...
app.whenReady().then(() => {
  ...
  protocol.registerFileProtocol("screenshot-image", (request, callback) => {
    try {
      ...
      console.error(
        "Failed to handle screenshot-image protocol request",
        error
      );
    }
  });
  ...
  ipcMain.handle("start-screenshot-mode", async () => {
    ...
    // Close any existing windows and clear previous interval
    windowManager.closeWindowsByType("screenshot");
    if (screenPollInterval) clearInterval(screenPollInterval);

    // Use tray position if available, otherwise fall back to cursor position
    ...
    if (!source) {
      console.error(
        `Could not find screen source for displayId: ${currentDisplay.id}`
      );
      return;
    }

    const image = await takeScreenshotForDisplay(
      source,
      "fullscreen",
      null,
      currentDisplay
    ...
    const win = windowManager.createWindow("screenshot", {
      ...
      params: {
        ...
        initialMouseX:
          screen.getCursorScreenPoint().x - currentDisplay.bounds.x,
        initialMouseY:
          screen.getCursorScreenPoint().y - currentDisplay.bounds.y,
      },
    });
    ...
    screenPollInterval = setInterval(async () => {
      try {
        ...
        if (win.currentDisplayId !== display.id) {
          ...
          if (newSource) {
            const newImage = await takeScreenshotForDisplay(
              newSource,
              "fullscreen",
              null,
              display
  ...
  ipcMain.handle("copy-screenshot", async (event, type, bounds, displayId) => {
    try {
      ...
      const targetDisplay =
        displays.find((d) => d.id.toString() === displayIdStr) ||
        primaryDisplay;
      ...
      if (!source && sources.length === displays.length) {
        ...
        const targetIndex = sortedDisplays.findIndex(
          (d) => d.id.toString() === displayIdStr
        );
      ...
      if (!source) {
        ...
        source =
          sources.find((s) => s.display_id === primaryDisplayIdStr) ||
          sources[0];
      }

      if (!source) {
        throw new Error(
          `Could not find any screen source for displayId: ${displayIdStr}`
        );
      }
  ...
  ipcMain.handle(
    "search-image-google",
    async (event, type, bounds, displayId) => {
      try {
        // Hide the screenshot overlay window to exclude it from the capture
        const senderWindow = BrowserWindow.fromWebContents(event.sender);
        if (senderWindow) {
    ...
    async (event, type, bounds, displayId) => {
      try {
        ...
        const display = displayId
          ? displays.find(
              (d) => d.id === displayId || d.id.toString() === displayId
            )
  ...
  ipcMain.on("connectivity-status", (event, data) => {
    console.log(
      `[Main] Connectivity status received: ${data.status} (${
...
// Handle protocol URLs (deeplinks)
  ...
  if (url.startsWith("snaplark://auth")) {
    ...
    try {
      ...
      if (mainWindow && mainWindow.webContents) {
        ...
        // Ensure window is still shown even on error
        if (mainWindow.isMinimized()) {
          mainWindow.restore();
        }
        mainWindow.show();
